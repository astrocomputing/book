...
This final case study chapter synthesized the concepts of workflow management and parallel execution by constructing a complete, end-to-end pipeline for **searching for transiting exoplanets in TESS data**. Leveraging the `lightkurve` library for core astronomical tasks, the chapter detailed the implementation of a workflow encompassing data retrieval, light curve extraction, systematic error correction, transit searching, and candidate vetting. The scientific goals (finding transit signals in many light curves) and necessary inputs (list of TIC IDs, configuration parameters) were defined. Each logical step of the pipeline was then broken down: **Step 1 (Data Retrieval/Preparation)** involved using `lightkurve` to search MAST and download TESS Target Pixel Files (TPFs) and perform initial aperture photometry and cleaning. **Step 2 (Detrending)** focused on applying methods like `lightkurve`'s `.flatten()` to remove instrumental systematics and produce normalized light curves. **Step 3 (Transit Search)** detailed running the computationally intensive Box Least Squares (BLS) algorithm using `lightkurve`'s `.to_periodogram(method='bls')` to detect periodic transit-like dips and extract key signal parameters (period, duration, depth, power/SNR). **Step 4 (Vetting/Output)** covered generating diagnostic plots (like phase-folded light curves) for promising candidates identified by BLS and creating summary output lists or tables.

The chapter then illustrated how this entire sequence could be implemented and automated using two different frameworks discussed previously: **Snakemake**, showcasing its rule-based definition in a `Snakefile` where dependencies are managed implicitly through input/output filename patterns and execution is orchestrated via the `snakemake` command; and **Dask**, demonstrating how its task-parallel capabilities (`dask.bag` or `dask.delayed`) allow distributing the processing of individual light curves (applying the sequence of analysis functions) across multiple cores or cluster nodes directly within a Python script, triggered by `.compute()`. Practical considerations for running these workflows, monitoring progress, ensuring intermediate steps completed successfully, managing outputs, and achieving reproducibility through environment control (as discussed in Chapter 69) were emphasized. The case study provided a concrete example of how the workflow principles and parallelization tools covered in Part XI can be applied to tackle real-world, large-scale data analysis challenges in modern astrophysics like searching for exoplanets in TESS data.
...
4.  **Köster, J. (Ed.). (n.d.).** *Snakemake Documentation*. Snakemake Readthedocs. Retrieved January 16, 2024, from [https://snakemake.readthedocs.io/en/stable/](https://snakemake.readthedocs.io/en/stable/)
    *(Documentation for Snakemake, used conceptually for implementing the workflow in Application 70.A.)*

5.  **Dask Development Team. (n.d.).** *Dask Documentation*. Dask. Retrieved January 16, 2024, from [https://docs.dask.org/en/latest/](https://docs.dask.org/en/latest/)
    *(Documentation for Dask, used conceptually for implementing the workflow in Application 70.B.)*
